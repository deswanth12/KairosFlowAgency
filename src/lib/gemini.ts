 ...
 */
export async function callGemini(prompt: string, maxTokens = 512): Promise<string | null> {
  if (!GEMINI_API_KEY) {
...
Return this exact JSON structure:
{
  ...
  "summary": <one sharp sentence, max 12 words, describing the lead value>,
...
Only return the raw JSON. No code fences.`;
  ...
  try {
    ...
    const parsed = JSON.parse(cleaned) as LeadScore;
    // Validate shape
    if (
