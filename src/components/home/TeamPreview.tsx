  ...
  return (
    <section id="team" className="bg-[#F7F7F4] text-[#111827] py-20 sm:py-28 px-4 sm:px-6 lg:px-8 border-b border-[#D9E0E5]">
      <div className="max-w-7xl mx-auto">
        ...
        <FadeInStagger staggerDelay={0.07} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5">
          {foundersData.map((member, idx) => {
            ...
            return (
              <FadeInItem key={member.id} className="h-full">
                <SpotlightCard
                  spotlightColor="rgba(184, 97, 58, 0.14)"
                  className={`group h-full flex flex-col justify-between p-6 bg-white border rounded-2xl shadow-subtle-card hover:shadow-hover-card transition-all duration-300 ${
                    isFounder
