...
export const TeamPreview: React.FC = () => {
  return (
    <section id="team" className="bg-corporate-offwhite text-corporate-text py-20 sm:py-28 px-4 sm:px-6 lg:px-8 border-b border-corporate-border">
            ...
            return (
              <div
                ...
                className={`group flex flex-col justify-between p-6 bg-white border rounded-2xl shadow-subtle-card hover:shadow-hover-card transition-all duration-300 ${
                  isFounder
                    ? 'border-corporate-blue bg-gradient-to-b from-white to-blue-50/30 ring-1 ring-corporate-blue/30'
                    : 'border-corporate-border hover:border-corporate-blue/40'
                }`}
              >
                <div>
                    ...
                    <div
                      className={`w-11 h-11 rounded-xl font-mono font-bold text-sm flex items-center justify-center border ${
                        isFounder
                          ...
                          : 'bg-corporate-softBlue text-corporate-dark border-blue-100 group-hover:border-corporate-blue'
                      } transition-colors`}
                    >
                      {initials}
                    </div>
                    {isFounder ? (
                      <span className="px-2.5 py-0.5 rounded-full bg-corporate-dark text-white text-[10px] font-bold uppercase font-mono tracking-wider flex items-center gap-1 shadow-sm">
                        Founder
                      </span>
                    ) : (
                  ...
                  {/* Name & Role */}
                  <h3 className="text-xl font-bold text-corporate-dark tracking-tight mb-1 group-hover:text-corporate-blue transition-colors font-display">
                    {member.name}
                  </h3>
                  <div className="text-xs font-mono text-corporate-blue font-semibold mb-4">
                    {member.role}
                  </div>

                  <p className="text-xs text-corporate-mutedText leading-relaxed mb-5">
                    {member.bio}
                  </p>

                  {/* Key Responsibilities List */}
                  <div className="space-y-1.5 mb-6 pt-4 border-t border-corporate-border">
                    <div className="text-[10px] font-mono uppercase text-corporate-mutedText tracking-wider font-semibold mb-2">
                      Key Responsibilities:
                    </div>
                    {member.responsibilities?.slice(0, 4).map((resp, rIdx) => (
                      <div key={rIdx} className="flex items-start gap-1.5 text-[11px] text-corporate-text">
                        <CheckCircle2 className="w-3 h-3 text-corporate-blue flex-shrink-0 mt-0.5" />
                        <span className="leading-tight">{resp}</span>
                      </div>
                    ))}
                    {(member.responsibilities?.length || 0) > 4 && (
                      <div className="text-[10px] font-mono text-corporate-mutedText pl-4">
                ...
                <div className="pt-4 border-t border-corporate-border flex items-center justify-between text-corporate-mutedText">
                  ...
                  <div className="flex items-center gap-2">
                    {member.socialLinks.email && (
                      <a
                        ...
                        className="p-1 hover:text-corporate-blue transition-colors"
