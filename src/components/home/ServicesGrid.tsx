  ...
  return (
    <section id="services" className="bg-[#F7F7F4] text-[#111827] py-20 sm:py-28 px-4 sm:px-6 lg:px-8 border-b border-[#D9E0E5]">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 6 Services Grid */}
        <FadeInStagger staggerDelay={0.08} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {servicesData.map((service, idx) => {
            const Icon = ICON_MAP[service.iconName] || Globe;
            const codeTag = SERVICE_CODES[service.slug] || `0${idx + 1} / CAPABILITY`;

            return (
              <FadeInItem key={service.id} className="h-full">
                <SpotlightCard
                  spotlightColor="rgba(184, 97, 58, 0.14)"
                  className="group relative h-full flex flex-col justify-between p-7 sm:p-8 rounded-2xl bg-white border border-[#D9E0E5] hover:border-[#B8613A]/40 shadow-subtle-card hover:shadow-hover-card transition-all duration-300"
                >
                  <div>
