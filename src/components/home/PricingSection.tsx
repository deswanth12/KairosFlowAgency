  ...
  return (
    <section id="pricing" className="bg-[#F7F7F4] text-[#111827] py-20 sm:py-28 px-4 sm:px-6 lg:px-8 border-b border-[#D9E0E5]">
      <div className="max-w-7xl mx-auto">
        ...
        <FadeInStagger staggerDelay={0.08} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {pricingTiersData.map((tier) => (
            <FadeInItem key={tier.id} className="h-full">
              <SpotlightCard
                spotlightColor="rgba(184, 97, 58, 0.15)"
                className={`h-full flex flex-col justify-between p-7 rounded-2xl bg-white border transition-all duration-300 relative ${
                  tier.popular
        ...
        <FadeIn direction="up" delay={0.2} className="mt-12 p-6 rounded-2xl bg-white border border-[#D9E0E5] flex flex-col sm:flex-row items-center justify-between gap-4 font-mono text-xs shadow-subtle-card">
